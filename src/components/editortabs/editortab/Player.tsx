...
export const Player: React.FC<PlayerProps> = ({ screens, selectedScreenId, characters, selectedCharacterId, scenarioId, onScreenSelect }) => {
  ...
  return (
    <>
      ...
      />
      <PanelGroup direction="vertical" autoSaveId="player-layout">
        ...
        {/* Дополнительная панель для будущего контента */}
        <Panel defaultSize={30} minSize={10}>
          ...
          >
            <Container sx={{ flex: 1, overflow: 'auto', p: 0 }}>
              {selectedScreen ? (
                <>
                  {selectedScreen.notes && (
                    <Typography variant="body2" sx={{ mb: 2 }}>
                      <strong>Notes:</strong> {selectedScreen.notes}
                    </Typography>
                  )}
                </>
              ) : (
                <Typography variant="body2" color="text.secondary">
                  Additional information panel - ready for future content
                </Typography>
              )}
            </Container>
          </Paper>
